...
def create_run_file(run_number, dc_offset, run, output_folder, output_file):
    if run==True:
...
def find_most_prominent_peak(file_path, temperature_range):
    ...
    if data.empty:
        ...
        return None
    # Detrend the data and magnitude calculation
    ...
    peak_position = data['Temperature (K)'].values[peaks][most_prominent_peak_idx]
    peak_prominence = prominences[most_prominent_peak_idx]
    print('----------------------------------------------')
    print('Peak detected @ '+ str(peak_position)+' with prominence '+str(peak_prominence)+' V')
    print('----------------------------------------------')
    ...
    while True:
        ...
        if new_data:
            ...
            if plot_counter == 0:
                ...
                line4.set_data(temperatures, np.sqrt(np.array(x2_vals)**2 + np.array(y2_vals)**2))
                line5.set_data(temperatures, x2_vals)
                line6.set_data(temperatures, y2_vals)
            ...
            if trend_counter == 10:
                ...
                if trend == "steady":
                    if not recording and abs(temperatures[-1]-temp_min)<=0.02:
                        ...
                        peak_data.append((current_dc_offset, find_most_prominent_peak(current_run_file, peak_range)))
                        ...
                        peak_data.append((current_dc_offset, peak_data[-3:][min_peak_idx][1]))#so that it works in consecutive runs
