...
def live_readout():
    ...
    ax5.set_ylabel('In-phase (V)')
    ax6.set_xlabel('Temperature (K)')
    ax6.set_ylabel('Out-of phase')
    ...
    while True:
        ...
        if new_data:
            ...
            if plot_counter == 0:
                ...
                line4.set_data(temperatures, magnitude(x2_vals, y2_vals))
                ...
                ax4.set_xlim(0, max(temperatures) + 1)
                ax4.set_ylim(0, magnitude(x2_vals, y2_vals))
                ax4.set_xticks([])

                ax5.set_xlim(0, max(temperatures) + 1)
                ax5.set_ylim(min(x2_vals), max(x2_vals))
                ax5.set_xticks([])

                ax6.set_xlim(0, max(temperatures) + 1)
                ax6.set_ylimmin((y2_vals), max(y2_vals))
            ...
            if trend_counter == 15:
                ...
                if trend == "steady":
                    if not recording and abs(temperatures[-1]-temp_min)<=0.01:
                        ...
                        recording = False
                        set_oscillator_parameters(lock_in_address, current_dc_offset, 10e-9, frequency)
