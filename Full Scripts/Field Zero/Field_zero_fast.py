...
def find_most_prominent_peak(file_path, temperature_range):
    ...
    print('----------------------------------------------')
    print('Peak detected @ '+ str("{:.3e}".format(peak_position))+' with prominence '+str("{:.3e}".format(peak_height))+' V')
    print('----------------------------------------------')
    return peak_position, peak_height, offset, std_dev
            ...
            if trend_counter == 10:
                ...
                if trend == "steady":
                    if not recording and abs(temperatures[-1]-temp_min)<=0.2:
                        ...
                        peak_position, peak_prominence, baseline, baseline_std =find_most_prominent_peak(current_run_file, peak_range)
                    ...
                    current_reading = np.polyfit(temperatures[-10:],R[-10:],0)[0]
                    plat_readings.append((current_dc_offset, current_reading))

                    if grid_state=='Change_range':
                            nahe = [t[1] for t in plat_readings[-3:]]
                            min_plat = np.argmin(nahe)
                            current_dc_offset = plat_readings[-3:][min_plat][0]
                            plat_readings.append((current_dc_offset, plat_readings[-3:][min_plat][1]))
                            range= dc_range[1]-dc_range[0]
                            dc_range[0] = current_dc_offset - 0.3*range
                            dc_range[1] = current_dc_offset + 0.3*range #~60% convergence of range
                            grid_state = '+'

                    if abs(temperatures[-1]-peak_position)<=0.1 and abs(current_reading-baseline)>=baseline_std:
                        current_dc_offset, grid_state =set_dc_offset(dc_range, grid_state, current_dc_offset)
                        set_oscillator_parameters(lock_in_address, current_dc_offset, ac_voltage, frequency) 
                    elif abs(temperatures[-1]-peak_position)<=0.1 and abs(current_reading-baseline)<=baseline_std:
                        print("Reading within baseline noise level, perform sweep")
                        grid_state = 'Secondary'  
