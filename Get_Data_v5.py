...
tolerance = 0.004 #tolerance for temperature change
...
#Takes in temperature data and tolerance and outputs trend
def detect_trend(temperatures, tolerance):
    
    last_60_temps = temperatures[-60:]
    
    if len(last_60_temps) < 2:
        return "Not enough data"  # Return "Not enough data" if there are not enough data points
    ...
    if abs(slope) < tolerance:
        return "Holding steady: " + str(datetime.now())
    elif slope > 0:
        return "Warming up: " + str(datetime.now())
    else:
        return "Cooling down: " + str(datetime.now())
...
def live_readout():
    ...
    #Matches the time stamps in the temperature log with lock-in readings from the 
    ...
    while True:
        ...
        if new_data:
            ...
            if plot_counter == 2:#Sets plot frequency
                # Update plot data
                ...
                plot_counter += 1
            if trend_counter==30:#Set time resolution for temperature check
                detect_trend(temperatures, tolerance)
            else:
                trend_counter+=1
