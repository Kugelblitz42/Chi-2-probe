...
def live_readout():
    ...
    ax1.plot([],[], 'b')
    ax1.set_title('Current Temperature:  (K)')
    ax2.plot([],[], 'g')
    ax2.set_title('Second Harmonic Magnitude')
    fig.show()
    ...
    while True:
        ...
        if latest_timestamp and latest_temperature:
            ...
            ax1.plot(np.subtract(timestamps,timestamps[0]), temperatures, 'b')
            ax1.set_title('Current Temperature: ' + str(latest_temperature) + ' (K)')

            ax2.plot(np.subtract(timestamps,timestamps[0]), magnitude(x2_vals, y2_vals), 'g')
